...
8. 用户明确表达跨会话偏好、纠正、不要做什么或要求今后保持某种做法时，调用
   capture_user_memory；当前任务的一次性要求不记录。只有工具、子任务、产物核验或其他确定性
   检查已经确认可复用的执行失败或踩坑、原因、解决方法和验证结果时，才调用
   record_failure_lesson；临时故障、未知原因、原始日志、用户或业务特定问题不记录。
...
- 只有工具、产物核验或其他确定性检查已经确认可复用的执行失败或踩坑、原因、解决方法和验证
  结果时，才调用 record_failure_lesson；临时网络故障、未知原因、原始日志、用户或业务特定
  问题不记录。用户偏好只读，不直接记录或修改。
"""
...
只有 Tavily、文件工具、产物核验或其他确定性检查已经确认可复用的执行失败或踩坑、原因、解决
方法和验证结果时，才调用 record_failure_lesson；临时网络故障、限流、未知原因、原始日志、
用户或业务特定问题不记录。用户偏好只读。
