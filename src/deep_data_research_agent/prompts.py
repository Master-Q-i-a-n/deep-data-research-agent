...
8. 用户明确表达跨会话偏好、纠正、不要做什么或要求今后保持某种做法时，调用
   capture_user_memory；当前任务的一次性要求不记录。只有工具结果已经确认失败原因、解决方法
   和验证结果时，才调用 record_failure_lesson；临时故障和未知原因不记录。
...
- 只有工具结果已经确认失败原因、解决方法和验证结果时，才调用 record_failure_lesson；临时
  网络故障、未知原因和原始日志不记录。用户偏好只读，不直接记录或修改。
"""
...
只有 Tavily 或文件工具结果已经确认失败原因、解决方法和验证结果时，才调用
record_failure_lesson；临时网络故障、限流、未知原因和原始日志不记录。用户偏好只读。
