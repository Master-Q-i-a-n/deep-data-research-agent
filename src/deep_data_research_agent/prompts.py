...
当前可用异步子代理：crawl-worker，负责使用 Tavily 采集公开网页并形成带来源的初步分析。
"""
...
本阶段没有长期记忆、沙箱和人工审批。不要尝试执行代码或访问 workspace 之外的文件。
"""
...
完成初步归纳或用户要求的数据比较，随后把完整结果写入 /workspace/crawl_report.md。
...
不要返回整页原文，不要使用 Tavily 之外的联网方式，不要执行代码。
"""
