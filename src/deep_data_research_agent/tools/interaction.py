...
_EMAIL_STATUS_MESSAGES = {
    "queued": "相同发送请求已在队列中，系统未重复创建。",
    "processing": "相同发送请求正在打包附件，系统未重复创建。",
    "retry": "相同发送请求正在等待安全重试，系统未重复创建。",
    "submitting": "相同发送请求正在提交 SMTP，系统未重复创建。",
    "sent": "相同发送请求已经完成，系统未重复发送。",
...
def _email_result(
    ...
    *,
    ...
    return ToolMessage(
        ...
        status=(
            "success"
            if status in {"queued", "processing", "retry", "submitting", "sent"}
            else "error"
        ),
    )
...
async def send_report_email(
    ...
    *,
    ...
    """Export one approved workspace and enqueue its durable email delivery."""
    ...
    try:
        ...
        pdf_filename = PurePosixPath(normalized_pdf_path).name
        zip_filename = f"{PurePosixPath(normalized_markdown_path).stem}-bundle.zip"
        ...
        try:
            await asyncio.to_thread(publish_email_delivery, idempotency_key)
        except Exception:
            # PostgreSQL is the outbox; maintenance will publish after recovery.
            logger.exception("发布邮件 Celery 任务失败，等待恢复扫描：%s", idempotency_key)
        return _email_result(
            runtime,
            status="queued",
            recipient=normalized_recipient,
            attachments=attachments,
            message="报告邮件已进入后台发送队列。",
