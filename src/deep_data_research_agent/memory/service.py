...
class MemoryQueue:
    """MongoDB-backed durable memory jobs and memory documents."""
    ...
    async def enqueue(
        ...
        *,
        ...
        if created:
            try:
                await asyncio.to_thread(publish_memory_job, str(result.upserted_id))
            except Exception:
                # MongoDB is the outbox. Beat republishes this pending job after
                # the Redis broker recovers, so accepting it here is still safe.
                logger.exception("发布记忆 Celery 任务失败，等待恢复扫描：%s", result.upserted_id)
        return created
    ...
    async def claim_job(self, job_id: str) -> dict[str, Any] | None:
        """Atomically claim one Celery-addressed job when it is due."""
    ...
    async def recoverable_job_ids(self, *, limit: int = 100) -> list[str]:
        """List durable jobs that should have a broker message now."""
    ...
    async def _process_job(self, job: dict[str, Any]) -> int | None:
        """Process one claimed job and return its retry delay when recoverable."""
