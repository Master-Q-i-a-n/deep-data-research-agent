...
def test_namespace_router_separates_skills_and_memories() -> None:
    ...
    user_namespace = ("user-a", "memories", "user")
    failure_namespace = ("public", "memories", "data-analyst")

    store.put(skill_namespace, "/active/demo/SKILL.md", {"content": "skill"})
    store.put(user_namespace, "/MEMORY.md", {"content": "preference"})
    store.put(failure_namespace, "/MEMORY.md", {"content": "failure"})

    assert skill_store.get(skill_namespace, "/active/demo/SKILL.md") is not None
    assert skill_store.get(user_namespace, "/MEMORY.md") is None
    assert memory_store.get(user_namespace, "/MEMORY.md") is not None
    assert memory_store.get(failure_namespace, "/MEMORY.md") is not None
    assert memory_store.get(skill_namespace, "/active/demo/SKILL.md") is None
...
async def test_namespace_router_supports_async_store_backend_calls() -> None:
    ...
    namespace = ("user-a", "memories", "user")

    await store.aput(namespace, "/MEMORY.md", {"content": "preference"})

    assert await memory_store.aget(namespace, "/MEMORY.md") is not None
    assert await skill_store.aget(namespace, "/MEMORY.md") is None
