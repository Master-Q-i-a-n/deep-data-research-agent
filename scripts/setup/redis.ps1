...
docker compose -f $composePath up -d --force-recreate redis
if ($LASTEXITCODE -ne 0) {
...
$celeryAuthenticated = (docker exec redis sh -c 'REDISCLI_AUTH="$(cat /run/secrets/redis_password)" redis-cli --user ddra-celery --no-auth-warning -n 1 PING').Trim()
if ($celeryAuthenticated -ne "PONG") {
    throw "Celery Redis ACL 认证验证失败"
}

Write-Output "Redis 已由 Compose 管理，数据卷 redis-data 已保留，应用与 Celery ACL 验证通过。"
